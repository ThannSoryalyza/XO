    ...
    @endif
                ...
                <div class="p-5">
                    <h2 class="text-2xl font-stadium text-gray-900 uppercase tracking-tight">
                        {{ $match->home_team }} <span class="text-red-600 italic text-lg"> vs </span> {{ $match->away_team }}
                    </h2>
                    <div class="mt-3 space-y-1">
                        <p class="text-gray-700 font-medium">
                           📍 {{ $match->stadium }} <span class="text-red-600 font-bold">({{ $match->location_type }}) </span>
                        </p>
                        <p class="text-gray-600">
                            <span class="font-bold text-gray-900">DATE:</span> {{ \Carbon\Carbon::parse($match->match_date)->format('d M Y') }}
                        </p>
                        <div class="flex gap-4">
                            <p class="text-gray-600"><span class="font-bold text-gray-900">KICK OFF:</span> {{ $match->match_time }}</p>
                            <p class="text-gray-600"><span class="font-bold text-gray-900">FINISH:</span> {{ $match->Finish_time }}</p>
                        </div>
                    </div>
                </div>
            </div>
